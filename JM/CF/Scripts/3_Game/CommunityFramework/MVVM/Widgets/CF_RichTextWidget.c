class CF_RichTextWidget : CF_TextWidget
{
	...
	void OnView_ContentOffset(CF_Model_Base model, CF_Event evt)
	{
		float _value = _RichTextWidget.GetContentOffset();
		EnScript.SetClassVar(model, ContentOffset, 0, _value);
	}

	void OnModel_ContentOffset(CF_Model_Base model, CF_Event evt)
	{
		float _value;
	...
	void OnView_ContentSnapToLine(CF_Model_Base model, CF_Event evt)
	{
		EnScript.SetClassVar(model, ContentSnapToLine, 0, _ContentSnapToLine);
	}

	void OnModel_ContentSnapToLine(CF_Model_Base model, CF_Event evt)
	{
	...
	void OnView_LineWidths(CF_Model_Base model, CF_Event evt)
	{
		array<float> _value;
	...
	void OnModel_LineWidths(CF_Model_Base model, CF_Event evt)
	{
