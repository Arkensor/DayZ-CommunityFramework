...
class CF_Widget : CF_MVVM_View
{
	...
	void OnView_Name(CF_Model_Base model, CF_Event evt)
	{
		string _name = _Widget.GetName();
		EnScript.SetClassVar(model, Name, 0, _name);
	}

	void OnModel_Name(CF_Model_Base model, CF_Event evt)
	{
		string _name;
	...
	void OnView_Visible(CF_Model_Base model, CF_Event evt)
	{
		bool _visible = _Widget.IsVisible();
		EnScript.SetClassVar(model, Visible, 0, _visible);
	}

	void OnModel_Visible(CF_Model_Base model, CF_Event evt)
	{
		bool _visible;
	...
	void OnView_Enabled(CF_Model_Base model, CF_Event evt)
	{
		EnScript.SetClassVar(model, Enabled, 0, _Enabled);
	}

	void OnModel_Enabled(CF_Model_Base model, CF_Event evt)
	{
		EnScript.GetClassVar(model, Enabled, 0, _Enabled);
		_Widget.Enable(_Enabled);
	}

	void OnView_Flags(CF_Model_Base model, CF_Event evt)
	{
		int _flags = _Widget.GetFlags();
		EnScript.SetClassVar(model, Flags, 0, _flags);
	}

	void OnModel_Flags(CF_Model_Base model, CF_Event evt)
	{
		int _flags;
	...
	void OnView_Sort(CF_Model_Base model, CF_Event evt)
	{
		int _sort = _Widget.GetSort();
		EnScript.SetClassVar(model, Sort, 0, _sort);
	}

	void OnModel_Sort(CF_Model_Base model, CF_Event evt)
	{
		int _sort;
	...
	void OnView_PositionX(CF_Model_Base model, CF_Event evt)
	{
		float _positionX;
		float _positionY;
	...
	void OnModel_PositionX(CF_Model_Base model, CF_Event evt)
	{
		float _positionX;
		float _positionY;
		_Widget.GetPos(_positionX, _positionY);
	...
	void OnView_PositionY(CF_Model_Base model, CF_Event evt)
	{
		float _positionX;
		float _positionY;
		_Widget.GetPos(_positionX, _positionY);
	...
	void OnModel_PositionY(CF_Model_Base model, CF_Event evt)
	{
		float _positionX;
		float _positionY;
		_Widget.GetPos(_positionX, _positionY);
	...
	void OnView_Width(CF_Model_Base model, CF_Event evt)
	{
		float _width;
		float _height;
	...
	void OnModel_Width(CF_Model_Base model, CF_Event evt)
	{
		float _width;
		float _height;
		_Widget.GetPos(_width, _height);
	...
	void OnView_Height(CF_Model_Base model, CF_Event evt)
	{
		float _width;
		float _height;
		_Widget.GetPos(_width, _height);
	...
	void OnModel_Height(CF_Model_Base model, CF_Event evt)
	{
		float _width;
		float _height;
		_Widget.GetPos(_width, _height);
	...
	void OnView_Color(CF_Model_Base model, CF_Event evt)
	{
		int _color = _Widget.GetColor();
		EnScript.SetClassVar(model, Color, 0, _color);
	}

	void OnModel_Color(CF_Model_Base model, CF_Event evt)
	{
		int _color;
	...
	void OnView_Roll(CF_Model_Base model, CF_Event evt)
	{
		EnScript.SetClassVar(model, Roll, 0, _Roll);
	}

	void OnModel_Roll(CF_Model_Base model, CF_Event evt)
	{
		EnScript.GetClassVar(model, Roll, 0, _Roll);
		_Widget.SetRotation(_Roll, _Pitch, _Yaw);
	}

	void OnView_Pitch(CF_Model_Base model, CF_Event evt)
	{
		EnScript.SetClassVar(model, Pitch, 0, _Pitch);
	}

	void OnModel_Pitch(CF_Model_Base model, CF_Event evt)
	{
		EnScript.GetClassVar(model, Pitch, 0, _Pitch);
		_Widget.SetRotation(_Roll, _Pitch, _Yaw);
	}

	void OnView_Yaw(CF_Model_Base model, CF_Event evt)
	{
		EnScript.SetClassVar(model, Yaw, 0, _Yaw);
	}

	void OnModel_Yaw(CF_Model_Base model, CF_Event evt)
	{
		EnScript.GetClassVar(model, Yaw, 0, _Yaw);
		_Widget.SetRotation(_Roll, _Pitch, _Yaw);
	}
	
	void OnView_Alpha(CF_Model_Base model, CF_Event evt)
	{
		int _alpha = _Widget.GetAlpha();
		EnScript.SetClassVar(model, Alpha, 0, _alpha);
	}

	void OnModel_Alpha(CF_Model_Base model, CF_Event evt)
	{
		int _alpha;
