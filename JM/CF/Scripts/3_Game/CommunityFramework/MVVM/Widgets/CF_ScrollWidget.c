class CF_ScrollWidget : CF_SpacerBaseWidget
{
	...
	void OnView_ScrollbarWidth(CF_ModelBase model, CF_Event evt)
	{
		EnScript.SetClassVar(model, ScrollbarWidth, 0, _ScrollWidget.GetScrollbarWidth());
	}

	void OnModel_ScrollbarWidth(CF_ModelBase model, CF_Event evt)
	{
		OnView_ScrollbarWidth(model, evt);
	}

	void OnView_ContentWidth(CF_ModelBase model, CF_Event evt)
	{
		EnScript.SetClassVar(model, ContentWidth, 0, _ScrollWidget.GetContentWidth());
	}

	void OnModel_ContentWidth(CF_ModelBase model, CF_Event evt)
	{
	...
	void OnView_HorizontalScrollPosition(CF_ModelBase model, CF_Event evt)
	{
		float _value = _ScrollWidget.GetHScrollPos();
		EnScript.SetClassVar(model, HorizontalScrollPosition, 0, _value);
	}

	void OnModel_HorizontalScrollPosition(CF_ModelBase model, CF_Event evt)
	{
		float _value;
	...
	void OnView_VerticalScrollPosition(CF_ModelBase model, CF_Event evt)
	{
		float _value = _ScrollWidget.GetVScrollPos();
		EnScript.SetClassVar(model, VerticalScrollPosition, 0, _value);
	}

	void OnModel_VerticalScrollPosition(CF_ModelBase model, CF_Event evt)
	{
		float _value;
