class CF_ItemPreviewWidget : CF_Widget
{
	...
	void OnView_Item(CF_ModelBase model, CF_Event evt)
	{
		EntityAI _value = _ItemPreviewWidget.GetItem();
		EnScript.SetClassVar(model, Item, 0, _value);
	}

	void OnModel_Item(CF_ModelBase model, CF_Event evt)
	{
		EntityAI _value;
	...
	void OnView_View(CF_ModelBase model, CF_Event evt)
	{
		int _value = _ItemPreviewWidget.GetView();
		EnScript.SetClassVar(model, View, 0, _value);
	}

	void OnModel_View(CF_ModelBase model, CF_Event evt)
	{
		int _value;
	...
	void OnView_ForceFlipEnable(CF_ModelBase model, CF_Event evt)
	{
		EnScript.SetClassVar(model, ForceFlipEnable, 0, _ForceFlipEnable);
	}

	void OnModel_ForceFlipEnable(CF_ModelBase model, CF_Event evt)
	{
		EnScript.GetClassVar(model, ForceFlipEnable, 0, _ForceFlipEnable);
		_ItemPreviewWidget.SetForceFlipEnable(_ForceFlipEnable);
	}

	void OnView_ForceFlip(CF_ModelBase model, CF_Event evt)
	{
		EnScript.SetClassVar(model, ForceFlip, 0, _ForceFlip);
	}

	void OnModel_ForceFlip(CF_ModelBase model, CF_Event evt)
	{
