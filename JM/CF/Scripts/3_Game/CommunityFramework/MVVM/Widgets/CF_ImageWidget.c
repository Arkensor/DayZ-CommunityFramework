class CF_ImageWidget : CF_Widget
{
	...
	void OnView_MaskTransitionWidth(CF_Model_Base model, CF_Event evt)
	{
		int _value = _ImageWidget.GetMaskTransitionWidth();
		EnScript.SetClassVar(model, MaskTransitionWidth, 0, _value);
	}

	void OnModel_MaskTransitionWidth(CF_Model_Base model, CF_Event evt)
	{
		float _value;
